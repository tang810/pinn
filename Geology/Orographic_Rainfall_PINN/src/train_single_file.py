...
PROJECT_NAME = "Orographic_Rainfall_PINN"

DATA_HASH = "33d5ff5e40ce429dac3a6d27e63ed912"
MODEL_HASH = "33d5ff5e40ce429dac3a6d27e63ed912"

DATA_PATH = os.path.expanduser(f"~/public/Resource/{DATA_HASH}/北京.json")
MODEL_PATH = os.path.expanduser(f"~/minio/Resource/{MODEL_HASH}/trained_model_orographic_rainfall_pinn.pt")
...
def _from_geojson(obj):
    ...
    terrain = 0.6 * np.sin(2.2 * lon_n) + 0.4 * np.cos(1.7 * lat_n)
    rainfall = 35.0 + 12.0 * np.maximum(terrain, 0.0) + 3.0 * np.sin(lon_n + lat_n) + 0.2 * arr[:, 2:3]
    return x.astype(np.float32), rainfall.astype(np.float32)
