...
PROJECT_NAME = "geochatbot"

DATA_HASH = "1aa40004e0cb4b4c99df12162f8417a5"
MODEL_HASH = "d17477263fc646a0843c89eea2688e5f"

DATA_PATH = os.path.expanduser(f"~/public/Resource/{DATA_HASH}")
MODEL_PATH = os.path.expanduser(f"~/public/Resource/{MODEL_HASH}/trained_model_geochatbot.pt")
...
def _from_geochem_csv(data_path):
    data = np.genfromtxt(data_path, delimiter=",", names=True, dtype=None, encoding="utf-8")
    names = [name.lower() for name in data.dtype.names or []]
    name_map = {name.lower(): name for name in data.dtype.names or []}
    if {"longitude", "latitude", "cu"}.issubset(names):
        lon = np.asarray(data[name_map["longitude"]], dtype=np.float32).reshape(-1, 1)
        lat = np.asarray(data[name_map["latitude"]], dtype=np.float32).reshape(-1, 1)
        cu = np.asarray(data[name_map["cu"]], dtype=np.float32).reshape(-1, 1)
        finite = np.isfinite(lon[:, 0]) & np.isfinite(lat[:, 0]) & np.isfinite(cu[:, 0])
        return np.concatenate([lon[finite], lat[finite]], axis=1), cu[finite]

    numeric_cols = []
    for name in data.dtype.names or []:
        try:
            col = np.asarray(data[name], dtype=np.float32).reshape(-1, 1)
        except Exception:
            continue
        if np.isfinite(col).any():
            numeric_cols.append(col)
    if len(numeric_cols) < 2:
        raise ValueError(f"No usable numeric columns found in {data_path}")
    arr = np.concatenate(numeric_cols, axis=1)
    finite = np.isfinite(arr).all(axis=1)
    return _from_matrix(arr[finite])
...
def load_dataset(data_path):
    ...
    if os.path.isdir(data_path):
        for name in ["Analytical_value_55K03.csv"]:
    ...
    if suffix in (".csv", ".txt"):
        if suffix == ".csv":
            try:
                return _from_geochem_csv(data_path)
            except Exception:
                pass
        try:
