    ...
    def __init__(self,
        ...
        if net_params:
            ...
            try:
                ...
                self.refine_load_chekcpoint(load_params, self.net)
                '''
                own_state = self.net.state_dict()
                for name, param in load_params.items():
        ...
        if net_params_1:
            ...
            try:
                ...
                self.refine_load_chekcpoint(load_params_1, self.net_1)
        ...
        if distributed:
            ...
            self.net, self.net_1, self.opt = accelerator.prepare(self.net, self.net_1, self.opt)
#        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.opt, T_max=1e6, eta_min=1e-6)
        
    def refine_load_chekcpoint(self, load_params, net):
    ...
    def init_train_dl(self, train_dl):
        self.train_dl = cycle(train_dl)
        x, y = self.get_next()
        self.init_vis_t(x, y)

    def init_vis_t(self, x_f, y_f):
        (_,_,_,e) = self.neural_net_u(x_f, y_f)
        self.vis_t_minus  = self.alpha_evm*torch.abs(e).detach().cpu().numpy()

    def get_next(self):
        x, y = next(self.train_dl)
        #x = x.float().to(device)
        #y = y.float().to(device)
    ...
    def set_eq_training_data(self,
        ...
        self.y_f = X[1]
        requires_grad = True
        x_f_batch = np.random.choice(self.x_f.reshape(-1), self.batchsize, replace=True)
        x_f = torch.tensor(x_f_batch.reshape(-1,1), requires_grad=requires_grad).float().to(device)
        y_f_batch = np.random.choice(self.y_f.reshape(-1), self.batchsize, replace=True)
        y_f = torch.tensor(y_f_batch.reshape(-1,1), requires_grad=requires_grad).float().to(device)
        if time:
    ...
    def get_eq_training_minibatch(self):
        #perm = torch.randperm(self.x_f.shape[0])
        #idx = perm[:self.batchsize]
        requires_grad = True
        x_f_batch = np.random.choice(self.x_f.reshape(-1), self.batchsize, replace=True)
        x_f_batch = torch.tensor(x_f_batch.reshape(-1,1), requires_grad=requires_grad).float().to(device)
        y_f_batch = np.random.choice(self.y_f.reshape(-1), self.batchsize, replace=True)
        y_f_batch = torch.tensor(y_f_batch.reshape(-1,1), requires_grad=requires_grad).float().to(device)
        return x_f_batch, y_f_batch
...
#        eq22 = (u*v_x + v*v_y) + p_y - 1.0/self.Re*(v_xx + v_yy)
    ...
    def fwd_computing_loss_2d(self, loss_mode='MSE'):
        ...
        #x, y = self.get_next()
        #x, y = self.get_eq_training_minibatch()

        (self.eq1_pred, self.eq2_pred,
         #self.eq3_pred, self.eq4_pred) = self.neural_net_equations(x, y)
        ...
        if loss_mode == 'MSE':
            ...
            self.loss_e = self.loss_eq1+self.loss_eq2+self.loss_eq3 + 1e-1*self.loss_eq4
    ...
    def train(self,
        ...
        return self.solve_Adam(self.fwd_computing_loss_2d, num_epoch, mini_counts, scheduler)
    ...
    def solve_Adam(self,
        ...
        for epoch_id in range(num_epoch+2):
            ...
            for count in range(int(mini_counts)):
                #print(mini_counts)
...
#                  self.scheduler.step()
            ...
            if epoch_id == 0 or epoch_id % 100 == 0:
    ...
    def print_log(self, loss, losses, epoch_id, num_epoch):
        ...
        print("epoch/num_epoch: ", epoch_id, "/", num_epoch,
              "loss[Adam]: %.3e"
        ...
        if (epoch_id) % self.checkpoint_freq == 0:
            alpha = self.alpha_evm
            torch.save(
                self.net.state_dict(),
                self.checkpoint_path + 'net_params_' +str(alpha)+ '_' + str(epoch_id) + '.pth')
            torch.save(
                self.net_1.state_dict(),
                self.checkpoint_path + 'net_params_' +str(alpha)+ '_' + str(epoch_id) + '.pth_evm')
...
#        div_pred = div_pred.detach().cpu().numpy().reshape(257,257)
        ...
        scipy.io.savemat('cavity_result_loop_1.mat',
                    {'U_pred':u_pred,
                     'V_pred':v_pred,
                     'P_pred':p_pred,
