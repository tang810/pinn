...
def _sympy_sdf_to_sdf(sdf, dx=0.0001):
    ...
    def _sdf(fn_sdf, sdf_inputs, dx):
        def sdf(invar, params, compute_sdf_derivatives=False):
            ...
            # Normalize inputs so lambdify always receives homogeneous shapes.
            # Expected shape for geometry variables is (N, 1).
            ref_shape = None
            ...
            if ref_shape is not None:
                for k, v in list(inputs.items()):
                    arr = np.asarray(v)
                    if arr.ndim == 0:
                        arr = np.full(ref_shape, arr, dtype=float)
                    elif arr.ndim == 1:
                        arr = arr.reshape(-1, 1)
                    elif arr.ndim >= 2 and arr.shape[1] != 1:
                        arr = arr.reshape(-1, 1)
                    if arr.shape != ref_shape:
                        arr = np.broadcast_to(arr, ref_shape)
                    inputs[k] = arr

            # Optional debug: enable by setting GEOM_DEBUG_SHAPE=1 in env.
            # if os.environ.get("GEOM_DEBUG_SHAPE", "0") == "1":
            #     print("DEBUG inputs:")
