...
try:
    ...
    def create_samples(indices, number_base=2):
        indices = np.asarray(indices).astype(np.int64).reshape(-1)
        out = np.zeros_like(indices, dtype=np.float64)
        for i, n in enumerate(indices):
            denom = 1.0
            vdc = 0.0
            while n > 0:
                n, rem = divmod(n, number_base)
                denom *= number_base
                vdc += rem / denom
            out[i] = vdc
        return out
...
SYMBOLIC_TYPES = (sympy.Basic, Parameter)
if symengine is not None:
    SYMBOLIC_TYPES = SYMBOLIC_TYPES + (symengine.Basic,)
...
class SympyCurve(Curve):
    ...
    def __init__(self, functions, parameterization, area, criteria=None):
        ...
        for key, func in functions.items():
            ...
            elif isinstance(func, SYMBOLIC_TYPES):
        ...
        if isinstance(area, float):
            area_fn = float(area)
        elif isinstance(area, SYMBOLIC_TYPES):
