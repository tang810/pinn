...
class PysicsInformedNeuralNetwork:
    ...
    def __init__(self,
        ...
        self.net_2 = ChebyKAN_2().to(device)
        ...
        if net_params_2:
            checkpoint_2 = torch.load(net_params_2, map_location=device)
            self.net_2.load_state_dict(checkpoint_2['model_state_dict'])
        ...
        if net_params and net_params_1:
            ...
            checkpoint_2 = torch.load(net_params_2)
            self.net.load_state_dict(checkpoint['model_state_dict'])
            self.net_1.load_state_dict(checkpoint_1['model_state_dict'])
            self.net_2.load_state_dict(checkpoint_2['model_state_dict'])
    ...
    def train(self,
        ...
        if self.opt is not None:
           self.opt.param_groups[0]['lr'] = lr
            ...
            self.opt = torch.optim.Adam(list(self.net.parameters()) + list(self.net_1.parameters()) + list(self.net_2.parameters()), lr=lr)
    ...
    def evaluate(self, x, y, u, v, t):
        ...
        plt.savefig('result_plot_evkan.png')
        plt.show()
        plt.close()
