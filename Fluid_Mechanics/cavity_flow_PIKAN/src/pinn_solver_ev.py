...
import scipy.io
import numpy as np
from src.model import ChebyKAN
from src.model import ChebyKAN_1
...
class PysicsInformedNeuralNetwork:
    ...
    def __init__(self,
        ...
        # 鍦ㄥ師浠ｇ爜鍩虹涓婁慨鏀?
        if net_params:
            print(f"Loading main network from: {net_params}")
            checkpoint = torch.load(net_params, map_location=device)
            self.net.load_state_dict(checkpoint['model_state_dict'])

            if 'model_1_state_dict' in checkpoint:
                 self.net_1.load_state_dict(checkpoint['model_1_state_dict'])
                 print("Successfully loaded auxiliary network net_1 from the same file.")
            else:
                 print("Warning: Auxiliary network net_1 not found in checkpoint. Using random weights.")


        # 璁剧疆浼樺寲鍣紝鍖呮嫭涓や釜缃戠粶鐨勫弬鏁?
    ...
    def solve_Adam(self,
        ...
        for epoch_id in range(num_epoch):
            ...
            if (epoch_id + 1) % num_epoch == 0:
                self.opt.state.clear()
                checkpoint = {'epoch': epoch_id + 1,
                    'epoch': epoch_id + 1,
                    'model_state_dict': self.net.state_dict(),
                    'model_1_state_dict': self.net_1.state_dict(),
                    'optimizer_state_dict': self.opt.state_dict(),
                    'loss': loss
                }
                os.makedirs(self.model_dir, exist_ok=True)
                save_path = os.path.join(self.model_dir, f"ev_net_checkpoint_epoch_{label}.pth")
                torch.save(checkpoint, save_path)
                print(f"\n--- Checkpoint saved to {save_path} at epoch {epoch_id + 1} ---\n")
    ...
    def evaluate(self, x, y, u, v):
        ...
        plt.savefig(os.path.join(self.result_dir, 'result_plot.png'))
        # plt.show()
        plt.close()
    ...
    def test(self, x, y, u, v, num_epoch, loop=None):
        ...
        result_folder = os.path.join(self.result_dir, 'result_uvdata')
