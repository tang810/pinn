...
class OpenAILLM(BaseLLM):
    ...
    async def _achat_completion_stream_response(self, messages: list[dict], timeout=3) -> str:
    ...
    async def acompletion_text(self, messages: list[dict], stream=False, timeout=3) -> str:
        ...
        if stream:
            rsp =  await self._achat_completion_stream(messages, timeout=timeout)
            return rsp
        else:
            rsp1 = await self._achat_completion(messages, timeout=timeout)
