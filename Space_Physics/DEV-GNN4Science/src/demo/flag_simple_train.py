...
def main():
    # configuration
    config = gnn4s.load_config('./config/flag_simple_train.yaml')
    ...
    process_fn = gnn4s.utils.partial(gnn4s.data.euler_lagrange_to_pyg_data,
                                     ...
                                     mask_noise_fn=mask_noise_fn, noise_std=config.data.noise_std, 
                                     noise_gamma=config.data.noise_gamma, label_update=True)
    ...
    trainer = gnn4s.train.Trainer(train_dataloader=dataloader, model=model, loss=loss,
                                  optimizer=optimizer, epoch_num=config.train.epoch_num,
