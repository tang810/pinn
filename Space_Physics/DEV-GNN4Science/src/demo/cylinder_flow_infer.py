...
def main():
    # configuration
    config = gnn4s.load_config('./config/cylinder_flow_infer.yaml')
    ...
    print(ckpt_config.model.block_num)
    ...
    process_fn = gnn4s.utils.partial(gnn4s.data.euler_lagrange_to_pyg_data, 
                                     sys_type='euler', mask_fn=mask_fn)
    ...
    model = gnn4s.model.MeshGraphNet(node_input_dim=ckpt_config.model.node_input_dim, 
                                     ...
                                     use_output_normalizer=ckpt_config.model.use_output_normalizer)
    model.load_state_dict(ckpt_dict['model_state_dict'])
    model.to(config.device)
    
    # evaluator
    predictor = gnn4s.infer.PredictorEuler(model, config.device)
    ...
    for data in dataloader:
        gnn4s.postprocess.plot_anime_trimesh2d(data, pred[0], config.output.name, config.output.dirt,
                                               skip=5, dpi=config.output.dpi)
        break
