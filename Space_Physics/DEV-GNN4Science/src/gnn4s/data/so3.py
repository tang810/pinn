...
def _score(exp, omega, eps, L=2000):  # score of density over SO(3)
    ...
    return dSigma / exp
'''
if os.path.exists('./dataset/pdbbind/processed/so3/.so3_omegas_array2.npy'):
    _omegas_array = np.load('./dataset/pdbbind/processed/so3/.so3_omegas_array2.npy')
    _cdf_vals = np.load('./dataset/pdbbind/processed/so3/.so3_cdf_vals2.npy')
    _score_norms = np.load('./dataset/pdbbind/processed/so3/.so3_score_norms2.npy')
    _exp_score_norms = np.load('./dataset/pdbbind/processed/so3/.so3_exp_score_norms2.npy')
else:
    print("Precomputing and saving to cache SO(3) distribution table")
    os.makedirs('./dataset/pdbbind/processed/so3', exist_ok=True)
    ...
    np.save('./dataset/pdbbind/processed/so3/.so3_omegas_array2.npy', _omegas_array)
    np.save('./dataset/pdbbind/processed/so3/.so3_cdf_vals2.npy', _cdf_vals)
    np.save('./dataset/pdbbind/processed/so3/.so3_score_norms2.npy', _score_norms)
    np.save('./dataset/pdbbind/processed/so3/.so3_exp_score_norms2.npy', _exp_score_norms)
'''
def get_omegas_array():
    if os.path.exists('./dataset/pdbbind/processed/so3/.so3_omegas_array2.npy'):
        _omegas_array = np.load('./dataset/pdbbind/processed/so3/.so3_omegas_array2.npy')
    else:
        print("Precomputing and saving to cache SO(3) distribution table")
        ...
        _omegas_array = np.linspace(0, np.pi, X_N + 1)[1:]
        np.save('./dataset/pdbbind/processed/so3/.so3_omegas_array2.npy', _omegas_array)
    return _omegas_array

def get_cdf_vals():
    if os.path.exists('./dataset/pdbbind/processed/so3/.so3_cdf_vals2.npy'):
        _cdf_vals = np.load('./dataset/pdbbind/processed/so3/.so3_cdf_vals2.npy')
    else:
        print("Precomputing and saving to cache SO(3) distribution table")
        os.makedirs('./dataset/pdbbind/processed/so3', exist_ok=True)
        _omegas_array = get_omegas_array()
        _eps_array = 10 ** np.linspace(np.log10(MIN_EPS), np.log10(MAX_EPS), N_EPS)
        _exp_vals = np.asarray([_expansion(_omegas_array, eps) for eps in _eps_array])
        _pdf_vals = np.asarray([_density(_exp, _omegas_array, marginal=True) for _exp in _exp_vals])
        _cdf_vals = np.asarray([_pdf.cumsum() / X_N * np.pi for _pdf in _pdf_vals])
...
def get_score_norms():
    if os.path.exists('./dataset/pdbbind/processed/so3/.so3_score_norms2.npy'):
        _score_norms = np.load('./dataset/pdbbind/processed/so3/.so3_score_norms2.npy')
    else:
        print("Precomputing and saving to cache SO(3) distribution table")
        os.makedirs('./dataset/pdbbind/processed/so3', exist_ok=True)
...
def get_exp_score_norms():
    if os.path.exists('./dataset/pdbbind/processed/so3/.so3_exp_score_norms2.npy'):
        _exp_score_norms = np.load('./dataset/pdbbind/processed/so3/.so3_exp_score_norms2.npy')
    else:
        print("Precomputing and saving to cache SO(3) distribution table")
        os.makedirs('./dataset/pdbbind/processed/so3', exist_ok=True)
        _omegas_array = get_omegas_array()
        _score_norms = get_score_norms()
        _eps_array = 10 ** np.linspace(np.log10(MIN_EPS), np.log10(MAX_EPS), N_EPS)
        _exp_vals = np.asarray([_expansion(_omegas_array, eps) for eps in _eps_array])
        _pdf_vals = np.asarray([_density(_exp, _omegas_array, marginal=True) for _exp in _exp_vals])
        _exp_score_norms = np.sqrt(np.sum(_score_norms**2 * _pdf_vals, axis=1) / np.sum(_pdf_vals, axis=1) / np.pi)
        np.save('./dataset/pdbbind/processed/so3/.so3_exp_score_norms2.npy', _exp_score_norms)
    return _exp_score_norms
...
def score_norm(eps):
    ...
    eps_idx = (np.log10(eps) - np.log10(MIN_EPS)) / (np.log10(MAX_EPS) - np.log10(MIN_EPS)) * N_EPS
    eps_idx = np.clip(np.around(eps_idx).astype(int), a_min=0, a_max=N_EPS-1)
    _exp_score_norms = get_exp_score_norms()
