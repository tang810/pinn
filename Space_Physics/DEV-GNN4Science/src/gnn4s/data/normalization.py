...
class Normalizer():
    def __init__(self, var_dict):
        """ initialization
        Args:
            var_dict: keys of the variables (postion, attibute and label of the nodes)
        """
        self.var_dict = var_dict

    def cal_mean_and_std(self, data_list):
        """ calculate mean and std of the node label """
        self.mean = {}
        self.std = {}
        for key in self.var_dict['node_label']:
            ...
            for data in data_list:
                self.mean[key] += data.node_label[key].sum(0)
                node_sum += data.node_label[key].shape[0]
            self.mean[key] /= node_sum

            for data in data_list:    
                self.std[key] += ((data.node_label[key]-self.mean[key])**2).sum(0)
            self.std[key] = (self.std[key]/node_sum)**0.5
    
    def load_state_dict(self, state_dict):
        """ load mean and std of the node label """
        self.mean = state_dict['mean']
        self.std = state_dict['std']
    
    def get_state_dict(self):
        """ get mean and std of the node label """
        state_dict = {}
    ...
    def normalize(self, data):
        ...
        for key in self.var_dict['node_label']:
            data.node_label[key] = (data.node_label[key]-self.mean[key]) / (self.std[key]+1e-8)
    
    def inverse_normalize(self, data):
        """ implement inverse normalization """
        for key in self.var_dict['node_label']:
            data[key] = data[key]*self.std[key] + self.mean[key]
'''
    ...
    for etype in fields:
            for field_name in fields[etype]:
                ...
                for graph_n in tqdm(graphs, desc = field_name, \
                    ...
                    # number of times
                    M = d.shape[2]
                    minv = np.min([minv, torch.min(d)])
                    maxv = np.max([maxv, torch.max(d)])
                    mean = torch.mean(d)
                    meansq = torch.mean(d**2)
...
def add_features(graphs, nodes_features = None, edges_features = None):
    ...
    for graph_n in tqdm(graphs, desc = 'Add features', colour='green'):
        ...
        add_feature(torch.ones(p.shape[0],1,ntimes) * torch.min(p), 
                    nodes_features, 
                    'dip')
        add_feature(torch.ones(p.shape[0],1,ntimes) * torch.max(p), 
                    nodes_features, 
                    'sysp')
        ...
        r1 = torch.zeros((nnodes,1,ntimes))
        c = torch.zeros((nnodes,1,ntimes))
        r2 = torch.zeros((nnodes,1,ntimes))
        ...
        cfeatures = torch.cat(cf, axis = 1)

        if 'loading' in nodes_features:
            loading = graph.ndata['loading']
            graph.ndata['nfeatures'] = torch.cat((p, q, cfeatures, loading), 
                                               axis = 1)
        else:
            graph.ndata['nfeatures'] = torch.cat((p, q, cfeatures), axis = 1)
        ...
        graph.edata['efeatures'] = torch.cat(cf, axis = 1)
...
def generate_normalized_graphs(graphs, norm_type, bc_type,
                               types_to_keep = None,
                               n_graphs_to_keep = -1,
                               statistics = None,
                               features = None):
    ...
    """
    fields_to_normalize = {'node': ['area','pressure','flowrate','dt'],
                           'edge': ['distance'],
    ...
    if docompute_statistics:
        norm_type = {'features': norm_type, 'labels': norm_type}
    ...
    return graphs, bc_type, statistics
'''