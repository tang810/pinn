"""
Notebook-friendly train file for airfoil_disp_pinn.

Online Jupyter usage:
1. Upload data.mat as a public asset.
2. Fill DATA_HASH and MODEL_HASH.
3. Run this whole file/cell. It saves trained_model_airfoil_disp.pt to
   ~/minio/Resource/{MODEL_HASH}/ for later public-asset upload.

Physics: 3D linear elasticity PINN (Navier-Lame eqns) for airfoil displacement.
...
MODEL_HASH = "8ba4730343624aa08babb31bfc9de5a7"
DATA_FILENAME = "data.mat"

DATA_PATH = os.path.expanduser(f"~/public/Resource/{DATA_HASH}")
MODEL_PATH = os.path.expanduser(f"~/minio/Resource/{MODEL_HASH}/trained_model_airfoil_disp.pt")
...
def resolve_data_path(data_path, data_hash=DATA_HASH, data_filename=DATA_FILENAME):
    """Resolve public asset paths to the uploaded .mat file."""
...
def show_train_result(pinn, eval_data, history):
    ...
    for idx, (ref, pred, title) in enumerate([
        (u_s, u_pred, "U"), (v_s, v_pred, "V"), (w_s, w_pred, "W"),
    ...
    plt.suptitle(f"Airfoil Disp — Ref vs PINN (L2: u={eu:.3e} v={ev:.3e} w={ew:.3e})", fontsize=14)
...
def train(data_path=DATA_PATH, model_path=MODEL_PATH, E=100, nu=0.3,
    ...
    print(f"boundary pts={bd[0].shape[0]}, interior pts={eq_d[0].shape[0]}, eval pts={eval_d[3].shape[0]}")

    pinn = AirfoilPINN(E=E, nu=nu, hidden=hidden, layers=layers, lr=lr)
