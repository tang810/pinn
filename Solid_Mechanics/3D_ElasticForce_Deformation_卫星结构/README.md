<!-- 
text："三维弹性力学应力与位移预测",
area: "准确预测固体结构在受力情况下的内部应力与变形，对于确保工程结构（如桥梁、航空航天部件、机械零件等）的安全可靠运行和优化设计至关重要。结构的响应行为通常由弹性力学偏微分方程描述。当结构几何、载荷或边界条件复杂时，求解这些方程可能非常具有挑战性。<br/>本项目致力于应用**物理信息神经网络 (PINN)** 技术，求解一个定义在三维域内的线性弹性力学问题。目标是预测该域内任意点的位移场 (u, v, w) 以及相应的应力张量 (σ) 分布。PINN 的核心思想是将控制物理定律（在此为 **Navier-Cauchy 弹性力学方程**）和边界条件直接嵌入到神经网络的损失函数中进行训练。",
tags: [ "三维弹性力学", "应力应变预测", "Navier-Cauchy方程", "结构分析"], 
-->
...
1. 进入秋月白页面，点击聊天框上方的**方程求解**图标（红框）

 ![](./viz/Figure1.png) 

2. 点击**上传文件**图标，上传数据文件，支持格式: CSV, TXT, NPZ。数据文件结构示例如下：

| ![](./viz/Figure2.png)   |      ![](./viz/Figure3.png)       |
| :-------------------------------------------------------: | :------------------------------: |
|                 **Fig.2 上传文件示例**                 | **Fig.3 数据结构示例** |

3. 如果没有数据文件，模型会自动生成仿真数据用于训练和推理。数据的字段统计表格如下：

- **总记录数**: 5000 条
- **字段数量**: 12 个
- **字段类型**: 数值型（x, y, z, u, v, w, fx, fy, fz, E, mu）和分类型（point_type）

#### 字段统计表格

| 字段名       | 数据类型   | 最小值               | 最大值               | 平均值（近似）       | 标准差（近似）       | 唯一值数量（如适用） | 备注                     |
|--------------|------------|----------------------|----------------------|----------------------|----------------------|----------------------|--------------------------|
| x            | 数值型     | 0.00552              | 0.99972              | 0.50                 | 0.29                 | -                    | 坐标值                   |
| y            | 数值型     | 0.01546              | 0.99664              | 0.50                 | 0.29                 | -                    | 坐标值                   |
| z            | 数值型     | 0.00695              | 0.99117              | 0.50                 | 0.29                 | -                    | 坐标值                   |
| u            | 数值型     | 5.52e-06             | 0.00099              | 0.00050              | 0.00029              | -                    | 微小位移或增量           |
| v            | 数值型     | 2.53e-06             | 0.00050              | 0.00025              | 0.00014              | -                    | 微小位移或增量           |
| w            | 数值型     | 1.39e-06             | 0.00020              | 0.00010              | 0.00006              | -                    | 微小位移或增量           |
| fx           | 数值型     | -0.30826             | 0.34289              | 0.00                 | 0.10                 | -                    | 力分量                   |
| fy           | 数值型     | -0.29118             | 0.30407              | 0.00                 | 0.10                 | -                    | 力分量                   |
| fz           | 数值型     | -0.24707             | 0.30966              | 0.00                 | 0.09                 | -                    | 力分量                   |
| E            | 数值型     | 1.0                  | 1.0                  | 1.0                  | 0.0                  | 1                    | 弹性模量（常数）         |
| mu           | 数值型     | 0.25                 | 0.25                 | 0.25                 | 0.0                  | 1                    | 泊松比（常数）           |
| point_type   | 分类型     | -                    | -                    | -                    | -                    | 1                    | 所有值为 "interior"      |

4. 在对话框中输入 **"我想进行3D弹性力学卫星结构受力分析与PINN建模"**,或类似问题

 ![](./viz/Figure4.png) 

5. 模型会先进行分析和规划，然后给出完整的 PINN 建模实现示例的python代码

 ![](./viz/Figure5.png) 

6. 最后会运行代码并输出PINN预测结果

| ![](./results/pinn_displacements.png)  |      ![](./results/pinn_stresses.png)      |
| :-------------------------------------------------------: | :------------------------------: |
|                 **Fig.6 位移云图**                 |   **Fig.7 应力云图**   |
...
模型的损失函数由以下两部分组成（具体数学公式请参见 **数学背景**章节）：
...
$$
l_x = \sum\bigl(s_{11}^{1u\,2} + s_{12}^{1u\,2} + s_{13}^{1u\,2} + s_{12}^{1b\,2} + s_{13}^{1b\,2}\bigr)
$$
...
$$
l_y = \sum\bigl(s_{22}^{2u\,2} + s_{12}^{2u\,2} + s_{23}^{2u\,2} + s_{12}^{2b\,2} + s_{23}^{2b\,2}\bigr)
$$

- **$l_z$**：沿 $z$ 平面上下表面的应力残差（包含已知载荷 $y$）

$$
l_z = \sum\bigl((s_{33}^{3u} - y)^2 + s_{23}^{3u\,2} + s_{13}^{3u\,2} + s_{23}^{3b\,2} + s_{13}^{3b\,2}\bigr)
$$
...
该组合损失函数既约束了内部的静力平衡，也保证了边界应力条件的精度，具体数学推导请参考**数学背景**章节。
...
```math
\bar r_x
= \frac{1}{N_{x\mathrm{bnd}}}
  \sum_{(x,y,z)\in\partial\Omega_x}
  \bigl|\mathbf{n}\!\cdot\!\sigma - \mathbf{t}_{\mathrm{presc}}\bigr|,
```
...
<!-- │   ├── 3D 弹性力学                                      # 🧩 3D_弹性力学：在3D_弹性力学项目中，作为根目录，用于组织该案例的全部数据、模型、源码与可视化结果内容。
│   │   ├── data                                         # 🗂️ 3D_弹性力学 data：在3D_弹性力学项目中，作为数据准备环节，用于存储几何网格、材料参数或用户提供的实验数据文件。
│   │   │   ├── data.csv                                 # 📄 3D_弹性力学 data.csv：在3D_弹性力学项目中，作为输入数据文件，提供用于训练的位移场或应力场实验数据。
│   │   ├── models                                       # 🗂️ 3D_弹性力学 models：在3D_弹性力学项目中，作为模型存储环节，用于保存训练完成的神经网络模型权重。
│   │   │   ├── winged_block_elasticity_model.pt         # 📄 3D_弹性力学 winged_block_elasticity_model.pt：在3D_弹性力学项目中，作为模型权重文件，保存基于PINN训练得到的翼块结构弹性分析模型参数。
│   │   ├── results                                      # 🗂️ 3D_弹性力学 results：在3D_弹性力学项目中，作为结果输出与可视化环节，用于保存预测数据、应力云图与位移场图等。
│   │   │   ├── numerical_results.npy                    # 🖼️ 3D_弹性力学 numerical_results.npy：在3D_弹性力学项目中，作为数值解数据文件，保存有限元求解的完整位移场(u,v,w)和坐标信息。
│   │   │   ├── training_results.npy                     # 🖼️ 3D_弹性力学 training_results.npy：在3D_弹性力学项目中，作为训练历史文件，保存PINN模型训练过程的损失函数变化和模型路径信息。
│   │   │   ├── quick_results.npy                        # 🖼️ 3D_弹性力学 quick_results.npy：在3D_弹性力学项目中，作为综合结果文件，整合数值解与PINN预测结果用于快速对比分析。
│   │   │   ├── numerical_u_displacement.png             # 🖼️ 3D_弹性力学 numerical_u_displacement.png：在3D_弹性力学项目中，作为数值解可视化图像，展示有限元计算的X方向位移分布云图。
│   │   │   ├── numerical_v_displacement.png             # 🖼️ 3D_弹性力学 numerical_v_displacement.png：在3D_弹性力学项目中，作为数值解可视化图像，展示有限元计算的Y方向位移分布云图
│   │   │   ├── numerical_w_displacement.png             # 🖼️ 3D_弹性力学 numerical_w_displacement.png：在3D_弹性力学项目中，作为数值解可视化图像，展示有限元计算的Z方向位移分布云图。
│   │   │   ├── pinn_u_displacement.png                  # 🖼️ 3D_弹性力学 pinn_u_displacement.png：在3D_弹性力学项目中，作为PINN预测图像，展示神经网络模型预测的X方向位移场分布。
│   │   │   ├── pinn_v_displacement.png                  # 🖼️ 3D_弹性力学 pinn_v_displacement.png：在3D_弹性力学项目中，作为PINN预测图像，展示神经网络模型预测的Y方向位移场分布。
│   │   │   ├── pinn_w_displacement.png                  # 🖼️ 3D_弹性力学 pinn_w_displacement.png：在3D_弹性力学项目中，作为PINN预测图像，展示神经网络模型预测的Z方向位移场分布。
│   │   │   ├── pinn_stress_s1.png                       # 🖼️ 3D_弹性力学 pinn_stress_s1.png：在3D_弹性力学项目中，作为应力分析图像，展示PINN模型预测的正应力σ11分量分布云图。
│   │   │   ├── pinn_stress_s2.png                       # 🖼️ 3D_弹性力学 pinn_stress_s2.png：在3D_弹性力学项目中，作为应力分析图像，展示PINN模型预测的正应力σ22分量分布云图。
│   │   │   ├── pinn_stress_s3.png                       # 🖼️ 3D_弹性力学 pinn_stress_s3.png：在3D_弹性力学项目中，作为应力分析图像，展示PINN模型预测的正应力σ33分量分布云图。
│   │   │   ├── training_loss.png                        # 🖼️ 3D_弹性力学 training_loss.png：在3D_弹性力学项目中，作为训练监控图像，展示PINN模型训练过程中损失函数的收敛曲线。
│   │   ├── src                                          # 🗂️ 3D_弹性力学 src：在3D_弹性力学项目中，作为工具函数模块，用于实现神经网络建模、数据处理、训练与可视化等功能。
│   │   │   ├── pinn_model.py                            # 📄 3D_弹性力学 pinn_model.py：在3D_弹性力学项目中，作为网络构建环节，定义MultiFNN模型结构与弹性力学PINN前向传播逻辑。
│   │   │   ├── runner.py                                # 📄 3D_弹性力学 runer.py：在3D_弹性力学项目中，作为运行流程控制模块，用于组织训练、预测与保存等逻辑。
│   │   │   ├── geometry_generator.py                    # 📄 3D_弹性力学 geometry_generator.py：在3D_弹性力学项目中，作为数据生成模块，创建训练和数值解算专用的三维几何网格与边界条件数。
│   │   │   ├── trainer.py                               # 📄 3D_弹性力学 trainer.py：在3D_弹性力学项目中，作为模型训练模块，配置弹性方程损失项、边界条件损失与优化器。
│   │   │   ├── postprocess.py                           # 📄 3D_弹性力学 postprocess.py：在3D_弹性力学项目中，作为结果处理环节，实现位移场可视化、应力计算与误差分析功能。
│   │   │   ├── data_utils.py                            # 📄 3D_弹性力学 data_utils.py：在3D_弹性力学项目中，作为数据工具模块，实现用户数据导入、虚拟数据生成、有效性验证及PyTorch训练数据集构建，支持PINN等建模全流程
│   │   ├── 3D_ElasticForce_deformation_PINN_main.ipynb  # 📄 3D_弹性力学 3D_ElasticForce_deformation_PINN_main.ipynb：在3D_弹性力学项目中，作为交互式教学环节，展示弹性力学PINN建模过程与结果分析的Jupyter笔记本。
│   │   ├── 3D_ElasticForce_deformation_PINN_main.py     # 🧠 3D_弹性力学 3D_ElasticForce_deformation_PINN_main.py：在3D_弹性力学项目中，作为主控脚本环节，整合训练、推理、可视化与模型保存的完整流程。
│   │   ├── requirements.txt                             # 📄 3D_弹性力学 requirements.txt：在3D_弹性力学项目中，作为依赖管理文件，列出项目所需的Python库版本信息。
│   │   ├── README.md                                    # 📄 3D_弹性力学 README.md：在3D_弹性力学项目中，作为说明文档环节，介绍三维弹性力学问题背景、PINN建模方法、数据说明与运行方式。 -->