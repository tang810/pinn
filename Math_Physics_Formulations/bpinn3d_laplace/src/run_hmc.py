...
ROOT = Path("/data/AI4PDE_CN/src/PINN4Science/Math_Physics_Formulations/bpinn3d_laplace/src")
...
PROJECT_ROOT = Path("/data/AI4PDE_CN/src/PINN4Science/Math_Physics_Formulations/bpinn3d_laplace")
    ...
    if fast:
        ...
        if has_samples or has_state:
            ...
            if has_samples:
                ...
                np.save(str(outdir / 'u_mean.npy'), u_samps.mean(axis=0))
                np.save(str(outdir / 'u_std.npy' ), u_samps.std(axis=0))
                np.save(str(outdir / 'f_mean.npy'), f_samps.mean(axis=0))
                np.save(str(outdir / 'f_std.npy' ), f_samps.std(axis=0))
            ...
            # 否则使用 posterior_mean_state_dict（无不确定性）
                ...
                if f_samps.ndim == 3: f_samps = f_samps[..., 0]
                np.save(str(outdir / 'u_mean.npy'), u_samps.mean(axis=0))
                np.save(str(outdir / 'u_std.npy' ), np.zeros_like(u_samps.mean(axis=0)))
                np.save(str(outdir / 'f_mean.npy'), f_samps.mean(axis=0))
                np.save(str(outdir / 'f_std.npy' ), np.zeros_like(f_samps.mean(axis=0)))
